...
class Operator():
    ...
    def init_data(self, seed=1):
        ...
        train_feeder = feeder.TrainFeeder(graph=self.graph_input, random_start=self.hyperparameters['augmentation']['random_start'], random_perturbation=self.hyperparameters['augmentation']['random_perturbation'], angle_candidate=[i for i in range(-self.hyperparameters['augmentation']['max_angle_candidate'], self.hyperparameters['augmentation']['max_angle_candidate']+1)], scale_candidate=[i/100 for i in range(int(self.hyperparameters['augmentation']['min_scale_candidate']*100), int(self.hyperparameters['augmentation']['max_scale_candidate']*100)+1)], translation_candidate=[i/100 for i in range(int(-self.hyperparameters['augmentation']['max_translation_candidate']*100), int(self.hyperparameters['augmentation']['max_translation_candidate']*100)+1)], roll_sequence=self.hyperparameters['augmentation']['roll_sequence'], input_temporal_resolution=self.hyperparameters['model']['input_temporal_resolution'], processed_data_dir=self.processed_data_dir, dataset=self.train_dataset, num_per_positive_sample=self.hyperparameters['balance']['train_num_per_positive_sample'], num_per_negative_sample=self.hyperparameters['balance']['train_num_per_negative_sample'], absolute=self.hyperparameters['features']['absolute'], relative=self.hyperparameters['features']['relative'], motion1=self.hyperparameters['features']['motion1'], motion2=self.hyperparameters['features']['motion2'], bone=self.hyperparameters['features']['bone'], bone_angle=self.hyperparameters['features']['bone_angle'])
        val_feeder = feeder.EvalFeeder(graph=self.graph_input, random_perturbation=False, input_temporal_resolution=self.hyperparameters['model']['input_temporal_resolution'], parts_distance=self.hyperparameters['evaluation']['parts_distance'], processed_data_dir=self.processed_data_dir, dataset=self.val_dataset, absolute=self.hyperparameters['features']['absolute'], relative=self.hyperparameters['features']['relative'], motion1=self.hyperparameters['features']['motion1'], motion2=self.hyperparameters['features']['motion2'], bone=self.hyperparameters['features']['bone'], bone_angle=self.hyperparameters['features']['bone_angle'])
